...
function reducer(state, action) {
  if (action.type === "isActive" && action.payload === false) {
    return { ...state };
  }
  switch (action.type) {
    case "isActive":
    ...
    case "payLoan":
      if (state.isLoanTaken && state.balance >= state.loan) {
        return {
          ...state,
          loan: 0,
          balance: state.balance - action.payload, //here is a problem
          isLoanTaken: false,
        };
      }
      return state;
    case "closeAccnt":
      if (!state.isLoanTaken && state.balance === 0) {
  ...
  return (
    <div className="App">
      ...
      <p>
        <button
          onClick={() => {
            dispatch({ type: "closeAccnt", payload: false });
          }}
          disabled={!isActive}
        >
          Close account
